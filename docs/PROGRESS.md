...
## Постоянная БД — развёртывание на сервере (в процессе) · 2026-08-18
...
- **Статус:** серверная установка на паузе — автоматический слой безопасности
  Claude Code (в VS Code) блокирует удалённое изменение сервера по SSH. Снимется
  после перезагрузки правила разрешения `Bash(ssh:*)` (перезапуск сессии) либо
  ручного прогона runbook владельцем.
- Следующий шаг после установки: задать `DATABASE_URL` (Secret) в Render →
  сервис пересоберётся уже на постоянную БД; проверить, что данные переживают
  редеплой.
