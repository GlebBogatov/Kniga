...
def question_check_prompt(question: str) -> str:
    return (
        ...
        "Хороший вопрос — конкретный, про решение или ситуацию («остаться в "
        "компании или искать работу в марте»), а не размытый («какая у меня "
        "судьба»). Отдельно определи, не является ли вопрос кризисным (о "
        "самоповреждении, суициде, остром психологическом кризисе).\n"
        'Ответь объектом с полями: quality (одно из "good", "vague", "yes_no_ok"), '
        "hint (если vague — 1–2 предложения, как переформулировать; иначе пустая "
        "строка), crisis (true, если вопрос кризисный, иначе false)."
    )
